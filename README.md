...
| 16 | [**FeynRL**](https://github.com/FeynRL-project/FeynRL)                                                     | *"What I cannot create, I do not understand." (Feynman).* A post-training framework that treats RL's fragility as a tooling problem: when a modest algorithmic change forces you to touch rollout code, orchestration, and distributed training all at once, too many ideas become too expensive to test. FeynRL separates work along three independent axes (algorithms, rollouts, orchestration) so that "a new loss is a new loss, a new rollout engine is a new rollout engine, and the rest keeps working." Supports SFT, DPO, PPO, GRPO, CISPO, and P3O in both synchronous and asynchronous modes, and folds in the practical implementations details usually omitted from others. Built on Ray, DeepSpeed, and vLLM. |
